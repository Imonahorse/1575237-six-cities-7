...
function AddComment() {
  ...
  const onInputChange = ({target}) => {
    ...
    useComment({...comment, rating: target.value})
  };
  const onTextChange = ({target}) => {
    if (target.value.length < 50) {
      target.setCustomValidity('The message must be more than 50 characters');
      return;
    }
    target.setCustomValidity('');

    useComment({...comment, text: target.value})
  }
  const onFormSubmit = (evt) => {
    evt.preventDefault();
    useComment({rating: '', text: ''});
  }

  const {text} = comment;

  return (
    <form onSubmit={(evt) => onFormSubmit(evt)} className="reviews__form form" action="#" method="post">
      <label className="reviews__label form__label" htmlFor="review">Your review</label>
      <div className="reviews__rating-form form__rating" onChange={(evt) => onInputChange(evt)}>
